...
async fn license_handler() -> (HeaderMap, Json<LicenseInfo>) {
    ...
    (h, Json(LicenseInfo {
        ...
        source_code: "https://github.com/ext-sakamoro/ALICE-DNS-SaaS".into(),
...
async fn rate_mw(
    ...
    let ok = {
        let mut e = s.rate_limiters.entry(uid).or_insert_with(|| TokenBucket::new(200000.0, 200000.0 / 3600.0));
        e.try_consume()
    };
